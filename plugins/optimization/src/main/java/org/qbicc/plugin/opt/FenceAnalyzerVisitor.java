...
public class FenceAnalyzerVisitor implements NodeVisitor<Void, Value, Node, Void, ValueHandle> {
    ...
    private static final Map<String, FunctionInfo> functionInfoMap = new HashMap<>();
    private static FunctionInfo functionInfo;
    private Map<BasicBlock, BlockInfo> blockInfoMap;
...
//    final TriableVisitor<Try, Void> triableVisitor = new Triables();
    ...
    private void init(BasicBlock entryBlock, String functionName) {
        ...
        blockInfoMap = new HashMap<BasicBlock, BlockInfo>();
     ...
     *
     */
    public static Set<Node> getIncoming(BasicBlock block, String functionName) {
     ...
     *
     */
    private static boolean getIncoming(BasicBlock block, String functionName, Set<Node> incomingSet, List<BasicBlock> trace) {
...
//        node.getDelegateOperation().accept(triableVisitor, node);
    ...
    */
    ...
    public Node visit(final Void param, Store node) {
        System.out.println("Store2=" + node + ", dep=" + node.getDependency() + ", " + Thread.currentThread());
    ...
    public Value visit(final Void param, Load node) {
        System.out.println("Load2=" + node + ", dep=" + node.getDependency() + ", " + Thread.currentThread());
...
//        System.out.println("Call=" + node + ", dep=" + node.getDependency());
        blockInfo.add(node);
        ...
        try {
            ...
            logger.debugf("No dependency for Call %s", node);
            System.out.println("No dependency for Call " + node);
            blockInfo.fail();
        }
    ...
    public Value visitUnknown(Void param, Value node) {
//	    System.out.println("UnknownValue:" + node);
	    return node;
    }

    public Node visitUnknown(Void param, Action node) {
//	    System.out.println("UnknownAction:" + node);
	    return null;
    }

    public Void visitUnknown(Void param, Terminator node) {
//	    System.out.println("UnknownTerminator:" + node);
	    return null;
    }

    /*
    public Value visit(final Try try_, final Call node) {
        logger.debugf("Call %s", node);
        blockInfo.add(node);
  ...
  */
	...
	public void addTailNode(Set<Node> set) {
        ...
        public boolean resolved() {
            return isFailedInfo || tailNodes != null;
        }
        ...
        public void setUnweakened(Node n) {
           if (unweakened == null) {
               unweakened = new HashSet<Node>();
           }

	   unweakened.add(n);
        }

        public boolean remainsOpportunity() {
            return unweakened != null;
        }
     ...
     * Store, and Call nodes. It holds 1) a list of these nodes stored in
    ...
    class Triables implements TriableVisitor<Try, Void> {
        public Void visit(final Try param, final Call node) {
            visitedValues.add(node);
            FenceAnalyzerVisitor.this.visit(param, node);
            return null;
        }
    }
	*/
